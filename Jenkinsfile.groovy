node('master') {
   ...
   def testServices(serviceName) {
      ...
      def command_val = 'curl -X POST -o ${WORKSPACE}/tests/'+serviceName+'_service.json -w %{response_code} --header "Content-Type: application/json" --header "Content-Type: application/json" --data '+single+' --insecure '+urlval
      respCode = sh (script: command_val, returnStdout: true)
