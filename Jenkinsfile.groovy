node('master') {
   ...
   def testServices(serviceName) {
      ...
      def command_val = 'curl -X POST -o ${WORKSPACE}/tests/'+serviceName+'_service.json -w %{response_code} --header "Content-Type: application/json" --header "Content-Type: application/json" --data '{"DFH0XCMNOperation":{"ca_request_id":"01INQS","ca_inquire_single":{"ca_item_ref_req":20}}}' --insecure '+urlval
      respCode = sh (script: command_val, returnStdout: true)
