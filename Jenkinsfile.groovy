node('master') {
   ...
   def installSar(sarFileName){
      ...
      //curl command spits out response code into stdout.  that's then held in respCode field to evaluate
       def command_val = "curl -X POST -o response.json -w %{response_code} --header 'Content-Type:application/zip' --data-binary @/archives/"+sarFileName+" --insecure "+urlval
   ...
   def testServices(serviceName) {
      ...
      def command_val = 'curl -X POST  -o tests/'+serviceName+'_service.json --header "Content-Type: application/json" --header "Content-Type: text/plain" --data tests/inquireSingle_service_request.json --insecure '+urlval
