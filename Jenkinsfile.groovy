node('master') {
   ...
   def testServices(serviceName) {
      ...
      def command_val = 'curl --location --request POST "'urlval+''+serviceName+'?action=invoke" --header "Content-Type: application/json" --header "Content-Type: text/plain" --data '+single+' -o tests/'+serviceName+'_service.json'
